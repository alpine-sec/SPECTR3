...
namespace ISCSI.Server
{
    ...
    public partial class ISCSIServer
    {
        ...
        public ISCSIServer()
        {
            PermitedIP = IPAddress.Any;
        }

        // Change the default value of PermitedIP to allow connections from other computers
        public ISCSIServer(IPAddress permitedIP)
        {
            PermitedIP = permitedIP;
        }
        ...
        private void ConnectRequestCallback(IAsyncResult ar)
        {
            ...
            {
                Console.WriteLine("    + Client Rejected from " + state.ConnectionParameters.InitiatorEndPoint.Address);
                SocketUtils.ReleaseSocket(clientSocket);
                m_listenerSocket.BeginAccept(ConnectRequestCallback, m_listenerSocket);
                return;
            }
